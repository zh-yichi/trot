"""
Example: LNO-AFQMC/pt2CCSD with trot.lnotrot
=============================================
...
Run from the repository root:  python trot/lnotrot/examples/lnotrot_pt2ccsd.py
"""
...
import trot.lnotrot  # noqa: F401  before jax: chooses the allocator that frees memory between fragments
...
mol = gto.M(atom="N 0 0 0; N 0 0 2", basis="sto-6g", spin=0, unit='b', verbose=4)
...
lno = LnoAfqmcMixed(
    ...
    target_error=1e-4,  # each fragment stops once its error < 0.7 * target / sqrt(nfrag),
...
mycc.kernel()
af = AfqmcMixed(mycc, trial="pt2ccsd_bar", norb_frozen_core=nfrozen, n_walkers=300, n_eql_blocks=80, n_blocks=300, seed=27)
e_ref, err_ref = af.kernel()
print(f"\nfull-space AFQMC/pt2CCSD E_corr = {e_ref - mf.e_tot:.6f} +/- {err_ref:.6f}")
print(f"LNO-AFQMC sum                   = {e_qmc:.6f} +/- {e_qmc_err:.6f}")
