...
class HamCholU:
    ...
    chol_a and chol_b must agree on n_fields, but norb_a and norb_b may differ. One
    common set of cholesky vectors L_g is projected into each spin's own orbital basis,

        L^sigma_g = C_sigma^dag L_g C_sigma,

    so the same g indexes both spins while the orbital dimensions are unrelated. This is
    what makes the opposite spin two body term <aa|bb> = sum_g L^a_g,pr L^b_g,qs
    recoverable, and is what unrestricted LNO produces, where the alpha and beta local
    active spaces are chosen independently.

    Contrast with HamChol, whose single chol/h1 forces both spins into one basis.
    """
...
def from_ham_chol(ham, *, h1_a=None, h1_b=None) -> HamCholU:
    ...
    Mainly for testing: the resulting propagator and kernels must reproduce the existing
    ("restricted" hamiltonian, "unrestricted" walker) path to roundoff.
    """
