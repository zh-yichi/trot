...
# The UCISD trial on the unrestricted (uchol) hamiltonian, where alpha and beta each
# live in their own orbital basis and the reference determinant of each spin occupies
# the leading nocc orbitals of that basis (build_ham_uchol's convention). The CI
# coefficients staged by staging._stage_ucisd_input are expressed exactly there: they are
# the UCCSD amplitudes in the UHF MO basis of each spin. So the same UcisdTrial pytree is
# reused, with mo_coeff_s the leading nocc columns of the identity of that spin's space;
# the kernels in meas.ucisd_uh never rotate.
...
def make_ucisd_trial_data_uh(data: dict, sys: Any) -> UcisdTrial:
    ...
    return UcisdTrial(
        mo_coeff_a=jnp.eye(norb_a)[:, :nocc_a],
        mo_coeff_b=jnp.eye(norb_b)[:, :nocc_b],
        c1a=c1a,
        c1b=c1b,
...
def get_rdm1_uh(trial_data: UcisdTrial) -> tuple[jax.Array, jax.Array]:
    ...
    it: it seeds the mean-field shift and the initial walkers, which is what afqmc's
    ucisd._calc_rdm1 does too.
    """
