...
from .chol_afqmc_ops import _make_vhs_split_flat, _sum_chol_squares
from .utils import taylor_expm_action

# low level details of AFQMC chol propagation with an unrestricted (uchol) hamiltonian.
#
...
#
# so norb_a and norb_b are free to differ. One sampled field vector x_g drives both spins
# through their own cholesky vectors, exp(i sqrt(dt) sum_g x_g L^s_g). See ham/chol_u.py.
#
# afqmc_step in prop/afqmc.py is reused unchanged: it only reads prop_ctx.chol_flat.shape[0]
# (the number of fields), mf_shifts, sqrt_dt, dt and h0_prop, all of which this context
# provides with the same meaning.
...
class CholAfqmcCtxU:
    ...
    def chol_flat(self) -> jax.Array:
        # what afqmc_step reads to size the field vector; alpha and beta agree on axis 0
...
def _mf_shifts_u(ham_data: HamCholU, rdm1) -> jax.Array:
    """
...
def _get_h1_eff_one_spin(h1: jax.Array, chol: jax.Array, mf: jax.Array) -> jax.Array:
    """
    h1_eff^s = h1^s - v0^s - v1^s

    v0 is spin diagonal (it contracts L^s with L^s), while v1 pairs the *total*
    mf_shifts with this spin's own cholesky vectors. _sum_chol_squares is the batched
    contraction the restricted propagator uses.
    """
    v0m = 0.5 * _sum_chol_squares(chol)
    mf_r = (1.0j * mf).real
