...
def test_u_sto_chol_kernel_full_head_exact_parts_and_unbiased(ufrag0):
    ...
    full = lmu.make_upt2ccsd_meas_ops(
        sys_, measure_type="sto_chol", nchol_chunk=3, n_chol_head="full"
    )
    sto = lmu.make_upt2ccsd_meas_ops(
...
def test_u_sto_chol_full_head_run_reproduces_bar_run(o2t, tmp_path):
    ...
    with contextlib.redirect_stdout(io.StringIO()):
        path = LnoFragMixed(o2t["mf"], o2t["frags"][0], chol_cut=CHOL_CUT).save(
            tmp_path / "frag1.h5"
        )
...
def test_ulno_afqmc_o2_triplet_end_to_end(o2t, tmp_path, request):
    ...
    with contextlib.redirect_stdout(io.StringIO()):
        af = AfqmcMixed(
            mycc, trial="upt2ccsd_bar", norb_frozen_core=nfrozen, seed=17, chol_cut=CHOL_CUT, **qmc
        )
